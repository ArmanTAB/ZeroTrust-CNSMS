// agent/src/modules/auth.js - Updated
...
const login = async (credentials, apiUrl) => {
  try {
    ...
    try {
      logger.info(`Attempting Agent API authentication for: ${credentials.email}`);
      ...
      if (response.data && response.data.success && response.data.token) {
        logger.info(`Agent API login successful for user: ${credentials.email}`);
      ...
      logger.info("Failed to authenticate via Agent API, falling back to MongoDB direct authentication");
    }
...
const authenticateDirectly = async (credentials, mongoUri) => {
  try {
    ...
    const result = await mongoDbDirect.authenticateUser(credentials.email, credentials.password);
    
    // If authentication is successful, get the rules for the user
    if (result.success) {
      logger.info(`Successfully authenticated user ${credentials.email} directly with MongoDB`);
