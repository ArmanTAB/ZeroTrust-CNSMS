...
interface EnhancedAccessLog {
  ...
  log_type: "access" | "drive_request" | "permission" | "agent";
  role?: string;
  granted_at?: string;
  granted_by?: string;
  revoked_at?: string;
...
const AccessLogsPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    start_time: "",
    end_time: "",
    log_type: "" as "" | "access" | "drive_request" | "permission" | "agent",
    user_email: "",
    folder_id: "",
    status: "",
  });
  ...
  const enhanceLogs = (logs: any[], type: string): EnhancedAccessLog[] => {
    return logs.map((log, index) => {
      if (log.log_type === "agent") {
        return {
          ...
          log_type: "agent" as
            ...
            | "permission"
            | "agent",
          clientId: `${type}_${log.id}_${index}`,
        };
      }
      ...
      return {
        ...log,
        log_type: type as "access" | "drive_request" | "permission" | "agent",
        clientId: `${type}_${log.id}_${index}`,
      };
    });
  };
  ...
  const fetchAllLogs = async () => {
    ...
    try {
      ...
      try {
        const accessLogs = await AccessApi.getAccessLogs({
          ...
          start_time: filters.start_time
            ...
            : undefined,
          end_time: filters.end_time ? new Date(filters.end_time) : undefined,
        });

        // Convert to enhanced format with unique keys
        const enhancedAccessLogs = accessLogs.map((log, index) => ({
          ...log,
          log_type: "access" as
            | "access"
            | "drive_request"
            | "permission"
            | "agent",
          clientId: `access_${log.id}_${index}`,
        }));
        combinedLogs = [...combinedLogs, ...enhancedAccessLogs];
      } catch (err) {
      ...
      // Fetch Drive requests
      try {
        const driveRequests = await AccessApi.getDriveAccessRequests(
          filters.status || ""
        );

        const enhancedDriveRequests = driveRequests.map((req, index) => ({
          id: req.id,
          clientId: `drive_request_${req.id}_${index}`,
          user_id: req.user_id,
          user_email: req.user_email,
          device_id: req.device_id || "N/A",
          ip_address: req.device_ip || "N/A",
        ...
        if (filters.user_email) {
          const filterEmail = filters.user_email.toLowerCase();
          combinedLogs = [
            ...combinedLogs,
            ...enhancedDriveRequests.filter((req) =>
              req.user_email?.toLowerCase().includes(filterEmail)
            ),
          ];
        } else {
      ...
      try {
        ...
        const enhancedPermissions = permissions.map((perm, index) => ({
          id: perm.id,
          clientId: `permission_${perm.id}_${index}`,
        ...
        if (filters.user_email) {
          ...
          combinedLogs = [
            ...combinedLogs,
            ...enhancedPermissions.filter((perm) =>
              perm.user_email?.toLowerCase().includes(filterEmail)
            ),
          ];
        } else {
      ...
      // Add agent logs - use the dedicated endpoint
      try {
        const agentLogs = await AccessApi.getAgentLogs({
          limit: 100,
          // Map filters
          action:
        ...
        console.log("Fetched agent logs:", agentLogs);

        if (agentLogs && agentLogs.length > 0) {
          const enhancedAgentLogs = agentLogs.map((log, index) => ({
            id: log.id,
            clientId: `agent_${log.id}_${index}`,
            log_type: "agent" as
              | "access"
              | "drive_request"
              | "permission"
              | "agent",
            // Map fields to match standard AccessLog interface structure
            device_id: log.deviceId || "N/A",
            user_id: log.userId || null,
            user_email: null, // Agent logs don't have this
            ip_address: null, // Agent logs don't have this
            user_agent: "Agent Software",
            ...
            access_granted: log.action !== "block",
          ...
          combinedLogs = [...combinedLogs, ...enhancedAgentLogs];
        }
      } catch (err) {
        console.error("Error fetching agent logs:", err);
        showToast(
          "Error fetching agent logs. Other log types will still be displayed.",
          "warning"
        );
      }

      // Filter by log type if specified in the filter
      if (filters.log_type) {
      ...
      const totalFilteredPages =
        Math.ceil(combinedLogs.length / itemsPerPage) || 1;
      ...
      const paginatedLogs = combinedLogs.slice(startIdx, endIdx);
  ...
  const applyFilters = (logsToFilter = allLogs) => {
    ...
    if (filters.device_id) {
      filteredLogs = filteredLogs.filter((log) => {
        if (log.log_type === "agent") {
          // For agent logs, check deviceId
          return log.device_id?.includes(filters.device_id);
        } else {
          return log.device_id?.includes(filters.device_id);
        }
      });
                      ...
                      >
                        ...
                        {log.log_type === "agent" && (
                          <div className="flex flex-col space-y-1">
                            <div className="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium border bg-purple-100 text-purple-800 border-purple-200">
                              <span className="mr-1">
                                <svg
                                  className="w-3 h-3"
                                  fill="none"
                            ...
                            <div>
                              ...
                              >
                                ...
                                ></span>
                                <span className="truncate max-w-[60px] sm:max-w-full">
                                  {log.access_granted ? "Allowed" : "Blocked"}
                                </span>
                              </span>
                            </div>
                          </div>
